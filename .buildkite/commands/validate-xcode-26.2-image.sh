...
# 3. watchOS 11.5 runtime available
...
echo "--- :iphone: Run xcodes runtimes"
if xcodes runtimes | grep -q "iOS 18.6 (Installed)"; then
  echo "✅ iOS 18.6 runtime is available"
  xcodes runtimes | grep "iOS 18"
else
...
echo "--- :watch: Checking watchOS 11.5 Runtime"
if xcrun simctl list runtimes | grep -q "watchOS 11.5"; then
  echo "✅ watchOS 11.5 runtime is available"
  xcrun simctl list runtimes | grep "watchOS 11"
else
  echo "❌ watchOS 11.5 runtime is NOT available"
  xcrun simctl list runtimes
  exit 1
fi
...
echo "Creating watchOS 11.5 simulator..."
xcrun simctl create "Test-watchOS11-Validation" "Apple Watch Series 9 (45mm)" "watchOS 11.5" || echo "Failed to create watchOS 11.5 simulator (may already exist)"

echo "--- :broom: Cleaning up test simulators"
xcrun simctl delete "Test-iOS18-Validation" || true
xcrun simctl delete "Test-watchOS11-Validation" || true
