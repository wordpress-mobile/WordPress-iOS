...
- (id)executeXMLRPCRequest:(XMLRPCRequest *)req byHandlingError:(BOOL)shouldHandleFalg {
	...
	if (httpAuthEnabled) {
		...
		//password wasn't being retrieved from keychain, this is a quick fix
		NSString *httpAuthPassword = [currentBlog valueForKey:@"authPassword"];
...
- (BOOL)syncPostsForCurrentBlog {
    ...
    if ([[currentBlog objectForKey:@"kIsSyncProcessRunning"] isEqual:[NSNumber numberWithInt:1]]) {
        NSLog(@"syncPosts already running, skipping");
        return NO;
    }
...
- (BOOL)syncIncrementallyLoadedPostsForCurrentBlog:(NSArray *)recentPostsList {
	...
	if ((success) && (shouldStopSyncingBlogs == NO)) {
		[self makeBlogAtIndexCurrent:currentBlogIndex];
		return YES;
	}else{
		return NO;
...
- (BOOL)organizePostsForBlog:(id)blog withPostsArray:(NSArray *) recentPostsList {
	...
	while (post = [postsEnum nextObject]) {
		...
		NSInteger sourceGMTOffset = [sourceTimeZone secondsFromGMTForDate:postGMTDate];
		NSInteger destinationGMTOffset = [destinationTimeZone secondsFromGMTForDate:postGMTDate];
		NSTimeInterval interval = destinationGMTOffset - sourceGMTOffset;
		
		NSDate* currentDate = [[[NSDate alloc] initWithTimeInterval:interval sinceDate:postGMTDate] autorelease];
		
		[post setValue:currentDate forKey:@"dateCreated"];
...
- (BOOL)makeAutoSavedPostCurrentForCurrentBlog {
    ...
    if (draftID) {
        ...
        NSString *postID = [NSString stringWithFormat:@"%@", [post valueForKey:@"postid"]];
...
- (BOOL)savePost:(id)aPost {
    ...
    if (currentPostIndex == -1 || ([postStatus intValue] == 1)) {
        ...
        NSDate *date = [aPost valueForKey:@"dateCreated"];
        NSInteger secs = [[NSTimeZone localTimeZone] secondsFromGMTForDate:date];
        NSDate *gmtDate = [date addTimeInterval:(secs * -1)];
