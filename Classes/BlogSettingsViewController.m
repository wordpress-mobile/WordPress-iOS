...
@synthesize tableView, recentItems, actionSheet, isSaving, viewDidMove, keyboardIsVisible, buttonText, recentItemsPopover, authUsername, authPassword;
...
- (void)viewWillDisappear:(BOOL)animated {
	...
	if (!isSaving){
		//save if user hits back button in navigation controller
		isSaving = YES;
		...
		case 1:
			...
			switch (indexPath.row) {
				...
				case 1:
					...
					authUsername = [[UITextField alloc] initWithFrame:textFieldFrame];
					authUsername.adjustsFontSizeToFitWidth = YES;
					authUsername.textColor = [UIColor blackColor];
					authUsername.placeholder = @"HTTP Auth Username";
					authUsername.text = [appDelegate.currentBlog objectForKey:@"authUsername"];
					authUsername.keyboardType = UIKeyboardTypeEmailAddress;
					authUsername.returnKeyType = UIReturnKeyDone;
					textCell.tag = 4;
					
					if(DeviceIsPad() == YES)
						authUsername.backgroundColor = [UIColor clearColor];
					else
						authUsername.backgroundColor = [UIColor whiteColor];
					
					authUsername.autocorrectionType = UITextAutocorrectionTypeNo;
					authUsername.autocapitalizationType = UITextAutocapitalizationTypeNone;
					authUsername.textAlignment = UITextAlignmentLeft;
					authUsername.delegate = self;
					
					authUsername.clearButtonMode = UITextFieldViewModeNever;
					[authUsername setEnabled: YES];
					
					[textCell addSubview:authUsername];
					cell = textCell;
					break;
				case 2:
					textCell.textLabel.text = @"Password";
					authPassword = [[UITextField alloc] initWithFrame:textFieldFrame];
					authPassword.adjustsFontSizeToFitWidth = YES;
					authPassword.textColor = [UIColor blackColor];
					authPassword.placeholder = @"HTTP Auth Password";
					authPassword.text = [appDelegate.currentBlog objectForKey:@"authPassword"];
					authPassword.keyboardType = UIKeyboardTypeDefault;
					authPassword.returnKeyType = UIReturnKeyDone;
					authPassword.secureTextEntry = YES;
					textCell.tag = 5;
					
					if(DeviceIsPad() == YES)
						authPassword.backgroundColor = [UIColor clearColor];
					else
						authPassword.backgroundColor = [UIColor whiteColor];
					
					authPassword.autocorrectionType = UITextAutocorrectionTypeNo;
					authPassword.autocapitalizationType = UITextAutocapitalizationTypeNone;
					authPassword.textAlignment = UITextAlignmentLeft;
					authPassword.delegate = self;
					
					authPassword.clearButtonMode = UITextFieldViewModeNever;
					[authPassword setEnabled: YES];
					
					[textCell addSubview:authPassword];
					cell = textCell;
					break;
				default:
					break;           
...
- (void)processRowValues {
	...
	[appDelegate.currentBlog setValue:authUsername.text forKey:@"authUsername"];
	[appDelegate.currentBlog setValue:authPassword.text forKey:@"authPassword"];
}
