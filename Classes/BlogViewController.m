...
#pragma mark -
#pragma mark iPad UITabBarController Methods
- (void)actionSheet:(CustomActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if (buttonIndex == 0) { 
		//discard
		tabBarController.selectedViewController = actionSheet.customObj;
		[postsViewController.postDetailViewController discard];
		[self tabBarController:tabBarController didSelectViewController:actionSheet.customObj];	
	} 
...
- (BOOL)showTabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController{
	BlogDataManager *dm = [BlogDataManager sharedDataManager];
...
- (BOOL)tabBarController:(UITabBarController *)tbController shouldSelectViewController:(UIViewController *)viewController {
	...
	if ( DeviceIsPad() == YES && tbController.selectedViewController == postsViewController 
		&& [postsViewController.postDetailViewController hasChanges]) {
		WordPressAppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
		CustomActionSheet *actionSheet = [[CustomActionSheet alloc] initWithTitle:@"You have unsaved changes."
														   ...
														   destructiveButtonTitle:@"Discard"
															otherButtonTitles:@"Cancel", nil];										  [actionSheet setCustomObj:viewController];
		actionSheet.actionSheetStyle = UIActionSheetStyleAutomatic;
		[actionSheet showInView:self.view];
		[appDelegate setAlertRunning:YES];
		return NO;
	} else
		return [self showTabBarController:tbController shouldSelectViewController:viewController];	
}
