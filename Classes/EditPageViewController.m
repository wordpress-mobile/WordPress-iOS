...
@synthesize contentTextView, selectedSection, titleTextField, isLocalDraft, originalTitle, originalStatus, originalContent, kbHeight;
@synthesize page, connection, urlRequest, urlResponse, payload, spinner, resignTextFieldButton, statusPopover, normalTableFrame, resizeContentArea; 
...
- (void)viewDidLoad {
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(stopEditing) name:@"EditPageViewShouldStopEditing" object:nil];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(resizeContentTextView) name:@"EditPageViewShouldResizeContentArea" object:nil];
	
	if(DeviceIsPad() == YES)
		[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewWillDisppearWithoutRollback:) name:@"EditPageViewIsDeleted" object:nil];
...
- (void)viewWillDisppear:(BOOL)animated {
	...
	@catch (NSException * e) {
		NSLog(@"viewWillDisppear error");
	}	
	[super viewWillDisappear:animated];
}

- (void)viewWillDisppearWithoutRollback:(BOOL)animated {
	[super viewWillDisappear:animated];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	switch (indexPath.section) {
		...
		case 1:
        {
			...
			contentCell.textView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
			contentCell.textView.frame = CGRectMake(0, 0, contentCell.frame.size.width, contentCell.frame.size.height);
...
- (void)textViewDidBeginEditing:(UITextView *)textView {
	delegate.isEditing = YES;
	selectedSection = [NSNumber numberWithInt:1];
	
	if ([self isShowingKeyboard]){
		UITableViewCell *cell = (UITableViewCell*) [[contentTextView superview] superview];
		...
		cell.frame = frame;
		[self.table bringSubviewToFront:cell];
...
- (void)setupPage {
	if(self.page == nil) {
		...
		if (self.isLocalDraft && DeviceIsPad())
			[[NSNotificationCenter defaultCenter] postNotificationName:@"HighlightPageCell" object:nil];
...
- (void)keyboardWillShow:(NSNotification *)notification {
	...
	if (delegate.isEditing){
		UITableViewCell *cell = (UITableViewCell*) [[contentTextView superview] superview];
		CGRect kb = [[[notification userInfo] objectForKey:UIKeyboardBoundsUserInfoKey] CGRectValue];
		CGRect frame = cell.frame;
		frame.size.height += 140;
		frame.size.height -= kb.size.height;
		kbHeight = kb.size.height;
...
- (void)resizeContentTextView {
	
	resizeContentArea = YES;
...
- (void)saveInBackground {
	...
	if(self.isLocalDraft == YES) {
		...
		if(delegate.selectedPostID == nil) {
			NSNumber *newPageID = [delegate.pageManager createPage:self.page];
            delegate.selectedPostID = [newPageID stringValue];
            [[NSNotificationCenter defaultCenter] postNotificationName:@"RefreshPageList" object:nil];
            self.page = nil;
            [self performSelectorOnMainThread:@selector(setupPage) withObject:self waitUntilDone:YES];
		} else {
...
- (void)insertMediaAbove:(NSNotification *)notification {
	...
	NSRange imgHTMLPre = [contentTextView.text rangeOfString:[NSString stringWithFormat:@"%@%@", @"<br/><br/>", content]];
	NSRange imgHTMLPost = [contentTextView.text rangeOfString:[NSString stringWithFormat:@"%@%@", content, @"<br/><br/>"]];
	if (imgHTMLPre.location == NSNotFound && imgHTMLPost.location == NSNotFound) {
		...
		NSMutableString *processedText = [[[NSMutableString alloc] initWithString:contentTextView.text] autorelease];
...
void logViewTree(UIView *view) {
    NSLog(@"[z=%f] %@", view.layer.zPosition, view);
    if (view.superview != nil) 
		logViewTree(view.superview);
}
