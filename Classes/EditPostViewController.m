...
@synthesize locationButton, locationSpinner, newCategoryBarButtonItem, autosaveButton, addURLSourceAlert;
...
- (void)refreshUIForAutosaveRestore: (Post *)post{
...
- (IBAction)endTextEnteringButtonAction:(id)sender {
    [textView resignFirstResponder];
	//#615 -- trick to rotate the interface. The shouldRotate... is not called when u click the done btn
	if (DeviceIsPad() == NO) {
		...
		{
			//	[[UIDevice currentDevice] setOrientation:UIInterfaceOrientationPortrait];  you can do this! this is a compiler Warning!!!
			UIViewController *garbageController = [[[UIViewController alloc] init] autorelease];
			[postDetailViewController.appDelegate.navigationController pushViewController:garbageController animated:NO];
			[postDetailViewController.appDelegate.navigationController popViewControllerAnimated:NO];
		}
	}
}
...
- (void)showLinkView {
    addURLSourceAlert = [[UIAlertView alloc] init];
	UIDeviceOrientation interfaceOrientation = [[UIDevice currentDevice] orientation];	
	if (DeviceIsPad() == NO && UIInterfaceOrientationIsLandscape(interfaceOrientation)) {
		infoText = [[UITextField alloc] initWithFrame:CGRectMake(12.0, 30.0, 260.0, 29.0)];
		urlField = [[UITextField alloc] initWithFrame:CGRectMake(12.0, 62.0, 260.0, 29.0)];
	} else {
...
- (void)textViewDidChange:(UITextView *)aTextView {
    ...
    NSArray *stringArray = [NSArray arrayWithObjects:@"http:", @"ftp:", @"https:", @"www.", @"<a hr", nil];
...
- (void)positionTextView:(NSDictionary *)keyboardInfo {
	...
	if(isEditing && postDetailViewController.isShowingKeyboard) {
		// Editing mode
		...
		if(DeviceIsPad() == YES) {
			if ((postDetailViewController.interfaceOrientation == UIDeviceOrientationLandscapeLeft)
				...
				// Landscape
                keyboardFrame = CGRectMake(0, 0, 703, 350);
				
				[textView setFrame:keyboardFrame];
			}
			else {
				// Portrait
                keyboardFrame = CGRectMake(0, 0, 768, 695);
		...
		if(DeviceIsPad() == YES) {
			if ((postDetailViewController.interfaceOrientation == UIDeviceOrientationLandscapeLeft)
				|| (postDetailViewController.interfaceOrientation == UIDeviceOrientationLandscapeRight)) {
				// Landscape
				keyboardFrame = CGRectMake(0, 180, 703, 475);
				
				[textView setFrame:keyboardFrame];
			}
			else if ((postDetailViewController.interfaceOrientation == UIDeviceOrientationPortrait) || (postDetailViewController.interfaceOrientation == UIDeviceOrientationPortraitUpsideDown)) {
				// Portrait
				keyboardFrame = CGRectMake(0, 180, 768, 730);
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
	...
	if (urlField != nil){
		if (textField = urlField){
			[addURLSourceAlert dismissWithClickedButtonIndex:2 animated:YES];
			[self saveLink];
			dismiss = YES;
			[delegate setAlertRunning:NO];
			[textView touchesBegan:nil withEvent:nil];
...
- (void)insertMediaAbove:(NSNotification *)notification {
	...
	if (imgHTMLPre.location == NSNotFound && imgHTMLPost.location == NSNotFound && imgHTML.location == NSNotFound) {
		...
		[content appendString:[NSString stringWithFormat:@"%@%@", prefix, processedText]];
		textView.text = content;
		postDetailViewController.hasChanges = YES;
	}
	
	//upgrade the data model
	[[[BlogDataManager sharedDataManager] currentPost] setObject:content forKey:@"description"];
}

- (void)insertMediaBelow:(NSNotification *)notification {
	NSLog(@"insertMediaBelow");
	Media *media = (Media *)[notification object];
	NSString *prefix = @"<br/><br/>";
...
- (void)removeMedia:(NSNotification *)notification {
	...
	//upgrade the data model
	[[[BlogDataManager sharedDataManager] currentPost] setObject:textView.text forKey:@"description"];
}
...
- (IBAction)showLocationMapView:(id)sender {
	...
	if (DeviceIsPad() == YES) {
		[delegate.splitViewController presentModalViewController:locationView animated:YES];
	}
	else {
...
- (void)keyboardWillShow:(NSNotification *)notification {
	...
	[postDetailViewController refreshButtons];
    [self positionTextView:keyboardInfo];
