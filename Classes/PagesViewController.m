...
- (void)viewDidLoad {
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshAfterSave) name:@"PagesUpdated" object:nil];
//	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loadPages) name:@"DidAddPage" object:nil];
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(highlightPageCell) name:@"HighlightPageCell" object:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(DeviceIsPad() == YES) {
		if (selectedIndexPath) {
			[[self.tableView cellForRowAtIndexPath:selectedIndexPath] setHighlighted: NO];
		}
...
- (void)highlightPageCell {
	//this is for trac #626
	[[self.tableView cellForRowAtIndexPath:selectedIndexPath] setHighlighted: YES];
}
...
- (void)didDeleteDraftAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (DeviceIsPad() == YES && self.selectedIndexPath != nil) {
		if (self.selectedIndexPath.section == indexPath.section && 
			self.selectedIndexPath.row == indexPath.row) {
		//deleted the current viewed page. push an empty page in the detailed screen.
			[self showAddNewPage];
		} else if(self.selectedIndexPath.section == 0 && [drafts count] == 0 ) {
			//deleted the last draft page, we show a blank page
			[self showAddNewPage];
		}
	}
