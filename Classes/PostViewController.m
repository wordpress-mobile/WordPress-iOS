...
- (void)startTimer;
- (void)stopTimer;
- (void)startAutoRecoveryTimer;
- (void)stopAutoRecoveryTimer;
- (void)discard;
- (void)cancel;
...
- (void)viewDidLoad {
	...
	//set preferences defaults for this class if the user hasn't visited App Settings yet
	NSString *autosave_frequency = [[NSUserDefaults standardUserDefaults] stringForKey: @"autosave_frequency_preference"];
	if(autosave_frequency == nil)
	{
		NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
		NSDictionary *appDefaults = [NSDictionary dictionaryWithObjectsAndKeys:
								 @"YES", @"autosave_enabled_preference",
								 @"5", @"autosave_frequency_preference",
								 nil];
...
- (void)viewWillAppear:(BOOL)animated {
	...
	[postPreviewController viewWillAppear:animated];
	if (DeviceIsPad() == NO) {
...
- (void)viewWillDisappear:(BOOL)animated {
	...
	[mediaViewController viewWillDisappear:animated];
...
- (void)tabBarController:(UITabBarController *)tabBarController didSelectViewController:(UIViewController *)viewController {
	...
	if (viewController == postPreviewController) {
		[postPreviewController setUpdatedPostDescription:postDetailEditController.textView.text]; //fix for #645
		[postPreviewController refreshWebView];
	} else {
	    [postPreviewController stopLoading];
	}
	//these methods are never called before the fixing of #628 because the tabcontroller delegate wasn't set.
	//so i've commented them, now that the delegate is properly set within the nib file.
...
- (IBAction)cancelView:(id)sender {
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"You have unsaved changes."
                                  delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:@"Discard Post"
...
- (IBAction)saveAction:(id)sender {
	[self.postDetailEditController.textView resignFirstResponder];
	...
	if((self.post == nil) || (self.post.isLocalDraft == [NSNumber numberWithInt:0])) {
		if ([[Reachability sharedReachability] internetConnectionStatus] == NotReachable) {
			...
			if (!hasChanges) {
				...
				else if([dm.currentPost valueForKey:@"dateCreated"] != nil){
					NSDate *date = [dm.currentPost valueForKey:@"date_created_gmt"];
					NSInteger secs = [[NSTimeZone localTimeZone] secondsFromGMT];
					NSDate *localDate = [date addTimeInterval:(secs)];
					[dm.currentPost setObject:localDate forKey:@"dateCreated"];
				}
...
- (void)autoSaveCurrentPost:(NSTimer *)aTimer {
	...
	if (hasChanges) {
		...
		if(postDetailEditController.categoriesTextField != nil)
			[autosave setCategories:postDetailEditController.categoriesTextField.text];
		if(postDetailEditController.tagsTextField != nil)
			[autosave setTags:postDetailEditController.tagsTextField.text];
...
- (void)startAutoRecoveryTimer {
	// this timer saves the post to file for autorecovery
    if ((!autoRecoverySaveTimer)) {
        autoRecoverySaveTimer = [[NSTimer scheduledTimerWithTimeInterval:10 target:self selector:@selector(saveAutoRecovery:) userInfo:nil repeats:YES] retain];
    }
}

- (void)stopAutoRecoveryTimer {
    if (autoRecoverySaveTimer) {
        [autoRecoverySaveTimer invalidate];
        [autoRecoverySaveTimer release];
        autoRecoverySaveTimer = nil;
    }
}

- (void)saveAutoRecovery:(NSTimer *)aTimer {
	[[[BlogDataManager sharedDataManager] currentPost] setObject:postDetailEditController.textView.text forKey:@"description"];
	[[BlogDataManager sharedDataManager] autoSaveCurrentPost];
}
...
- (void)publish:(id)sender {
	...
	//set the status of the post as published only if it a local draft
	if([[[dm.currentPost valueForKey:@"post_status"] lowercaseString] isEqualToString:@"local draft"]){
		...
		postDetailEditController.statusTextField.text = [dm.currentPost valueForKey:@"post_status"];
	}
...
- (void)restoreFromAutosave:(NSNotification *)notification {
	...
	if(autosave != nil) {
		if([autosave.isPublished isEqualToNumber:[NSNumber numberWithInt:0]]) {
			...
			[postDetailEditController refreshUIForAutosaveRestore: post];
		}
		else {
