...
- (void)viewDidLoad {
	...
	if(DeviceIsPad() == YES) {
		newButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCompose
																	  target:self
																	  action:@selector(showAddPostViewPreCheckForIPad)];
	} else {
		newButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCompose
																	  target:self
...
- (void)viewWillAppear:(BOOL)animated {
	...
	//reset the postDetailViewController
	if(postDetailViewController != nil){
		
		if(postDetailViewController.postDetailViewController != nil)
			[postDetailViewController.postDetailViewController viewWillDisappear:NO];
		
		if(postDetailViewController.mediaViewController != nil)
			[postDetailViewController.mediaViewController removeNotifications];
		
		if(postDetailViewController.postDetailEditController != nil)
			[postDetailViewController.postDetailEditController viewWillDisappear:NO];
	...
	{
		if ([defaults boolForKey:@"refreshPostsRequired"] || [[BlogDataManager sharedDataManager] countOfPostTitles] == 0) {
			[self refreshHandler];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	...
	else if ([defaults boolForKey:@"anyMorePosts"] && count >= 10) {
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)aTableView editingStyleForRowAtIndexPath:(NSIndexPath *)indexPath {
	// Detemine if we are on iPad and we are editing content on the detail controller
	if (DeviceIsPad() == YES && postDetailViewController != nil && postDetailViewController.hasChanges) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if(DeviceIsPad() == YES && postDetailViewController != nil && postDetailViewController.hasChanges){
		CustomActionSheet *actionSheet = [[CustomActionSheet alloc] initWithTitle:@"You have unsaved changes."
																		 delegate:self 
																cancelButtonTitle:nil 
														   destructiveButtonTitle:@"Discard"
																 otherButtonTitles:@"Cancel", nil];
		[actionSheet setCustomObj:indexPath];
		actionSheet.tag = 100;
	...
	[self showPostAtIndexPath:indexPath];
}
...
- (void)actionSheet:(CustomActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	WordPressAppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
	if(actionSheet.tag == 200) { //The new post btn on ipad clicked
		switch (buttonIndex) {
			case 0:
				//discard
				[postDetailViewController discard];
	...
	} else { //clicked on an item within the navigation controller
		NSIndexPath *indexPath = (NSIndexPath*)actionSheet.customObj;
...
- (void)showPostAtIndexPath:(NSIndexPath *)indexPath{
...
-(void)showAddPostViewPreCheckForIPad{
	if(postDetailViewController.hasChanges) {
