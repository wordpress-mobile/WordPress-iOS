...
- (void)sendXMLRPC {
	...
	if (httpAuthEnabled) {  
		[request setUsername:httpAuthUsername];  
		[request setPassword:httpAuthPassword];  
	}  
...
- (void)requestFinished:(ASIHTTPRequest *)req {
	...
	if(![[request responseString] isEmpty] && [request responseString] != nil) {
		@try{
			if ([[request responseString] rangeOfString:@"AtomPub services are disabled"].location != NSNotFound){
				[errorMsg setObject:@"err_gen" forKey:@"err_type"];
				[errorMsg setObject:[request responseString] forKey:@"err_msg"];
				if(mediaType == kVideo)
					[self finishWithNotificationName:VideoUploadFailed object:nil userInfo:errorMsg];
				...
				if ([xmlrpcResponse isKindOfClass:[NSError class]]) {
					...
					if ([faultString rangeOfString:@"Invalid file type"].location == NSNotFound){
						[errorMsg setObject:@"err_gen" forKey:@"err_type"];
						[errorMsg setObject:faultString forKey:@"err_msg"];
					}
					else {
						[errorMsg setObject:@"err_vp" forKey:@"err_type"];
						[errorMsg setObject:@"You can upload videos to your blog with VideoPress. Would you like to learn more about VideoPress now?" forKey:@"err_msg"];
					}
					if(mediaType == kVideo)
						[self finishWithNotificationName:VideoUploadFailed object:nil userInfo:errorMsg];
					else if(mediaType == kImage)
						[self finishWithNotificationName:ImageUploadFailed object:nil userInfo:errorMsg];
