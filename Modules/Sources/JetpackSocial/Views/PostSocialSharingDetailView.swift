...
public struct PostSocialSharingDetailView: View {
    ...
    private func bindingForToggle(connection: SocialConnection) -> Binding<Bool> {
        Binding(
            get: { !draft.disabledConnectionIDs.contains(connection.id) },
            set: { isEnabled in
                if isEnabled {
                    draft.disabledConnectionIDs.remove(connection.id)
                } else {
                    draft.disabledConnectionIDs.insert(connection.id)
                }
            }
        )
    }
...
extension PostSocialSharingDraft {
    ...
    public func summary(for connections: [SocialConnection]) -> String? {
        ...
        let enabledCount = connections.filter { !disabledConnectionIDs.contains($0.id) }.count
...
#if DEBUG
#Preview("Empty") {
    @Previewable @State var draft = PostSocialSharingDraft()
    return NavigationStack {
        PostSocialSharingDetailView(
            connections: SiteSocialConnectionsService.preview(connections: []),
            draft: $draft,
            onAddConnection: {}
        )
    }
