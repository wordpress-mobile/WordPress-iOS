...
enum TestHelpers {
    ...
    static func verifyExcerptsWordCount(
        ...
        // Lenient bounds: allow 30% below min, 50% above max before failing
        let strictMinWords = Int(Double(wordRange.lowerBound) * 0.7)  // 70% of minimum
        let strictMaxWords = Int(Double(wordRange.upperBound) * 1.5)  // 150% of maximum
    ...
    static func verifyTagsLanguage(_ tags: [String], expectedLanguage: NLLanguage) {
        for (index, tag) in tags.enumerated() {
            ...
            #expect(
                detectedLanguage == expectedLanguage,
                "Tag \(index + 1) language mismatch: expected \(expectedLanguage.rawValue), got \(detectedLanguage?.rawValue ?? "unknown")\nTag: \(tag)"
            )
        }
    }
