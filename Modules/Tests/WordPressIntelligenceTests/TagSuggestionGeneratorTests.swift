...
struct TagSuggestionGeneratorTests {
    ...
    @available(iOS 26, *)
    @Test(arguments: TagTestCaseParameters.allCases)
    func tagSuggestion(parameters: TagTestCaseParameters) async throws {
    ...
    func excludeExistingTags() async throws {
        let parameters = TagTestCaseParameters(
            data: TestData.spanishPost,
            siteTags: TestData.spanishSiteTags,
            postTags: ["recetas", "cocina"]
        )
...
struct TagTestCaseParameters: CustomTestStringConvertible {
    ...
    static let allCases: [TagTestCaseParameters] = [
        // English
        ...
        // Russian
        TagTestCaseParameters(data: Data.russianPost, siteTags: Data.russianSiteTags, postTags: []),
    ]
}
