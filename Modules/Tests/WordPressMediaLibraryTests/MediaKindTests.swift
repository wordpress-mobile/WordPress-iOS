...
struct MediaKindTests {
    ...
    @Test func mediaTypeParamMappingCoversAllKinds() {
        #expect(MediaKind.image.asMediaTypeParam == .image)
        #expect(MediaKind.video.asMediaTypeParam == .video)
        #expect(MediaKind.audio.asMediaTypeParam == .audio)
        #expect(MediaKind.document.asMediaTypeParam == .application)
    }
}
