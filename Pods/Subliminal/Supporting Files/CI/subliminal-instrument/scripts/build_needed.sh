...
# `build_needed.sh` echoes `0` if there exists an up-to-date build
# of `subliminal-instrument`, `1` otherwise.
#
# `build_needed.sh` is modeled after a similar script in https://github.com/facebook/xctool.
...
# We need to build unless there are no changes and we've already built
if [[ $HAS_CHANGES == "NO" && -e "$SI_PATH" ]]; then
    echo 0
else
    echo 1
fi
