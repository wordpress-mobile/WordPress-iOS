...
@implementation GTLFetcher (GTLServiceTicketAdditions)
- (id)ticket {
...
@interface GTLUploadFetcherClass : GTLFetcher
+ (instancetype)uploadFetcherWithRequest:(NSURLRequest *)request
                              uploadData:(NSData *)data
                          uploadMIMEType:(NSString *)uploadMIMEType
                               chunkSize:(NSUInteger)chunkSize
                          fetcherService:(GTLFetcherService *)fetcherService;
+ (instancetype)uploadFetcherWithRequest:(NSURLRequest *)request
...
+ (instancetype)uploadFetcherWithRequest:(NSURLRequest *)request
                           uploadFileURL:(NSURL *)uploadFileURL
                          uploadMIMEType:(NSString *)uploadMIMEType
                               chunkSize:(NSUInteger)chunkSize
                          fetcherService:(GTLFetcherService *)fetcherService;
+ (instancetype)uploadFetcherWithLocation:(NSURL *)locationURL
...
- (void)prepareToParseObjectForFetcher:(GTLFetcher *)fetcher;
- (void)handleParsedObjectForFetcher:(GTLFetcher *)fetcher;
...
- (BOOL)invokeRetrySelector:(SEL)retrySelector
                  ...
                  willRetry:(BOOL)willRetry
                      error:(NSError *)error;
- (BOOL)objectFetcher:(GTLFetcher *)fetcher
            willRetry:(BOOL)willRetry
             forError:(NSError *)error;
- (void)objectFetcher:(GTLFetcher *)fetcher
     finishedWithData:(NSData *)data
                error:(NSError *)error;
- (void)parseObjectFromDataOfFetcher:(GTLFetcher *)fetcher;
@end
...
- (id)init {
  ...
  if (self) {
    ...
    fetcherService_ = [[GTLFetcherService alloc] init];
...
- (NSString *)requestUserAgent {
  ...
  if ([userAgent length] == 0) {
    ...
    userAgent = GTLApplicationIdentifier(owningBundle);
  }
  ...
  if (libRange.location == NSNotFound) {
    ...
    NSString *systemString = GTLSystemVersionString();
...
- (GTLServiceTicket *)fetchObjectWithURL:(NSURL *)targetURL
  ...
  GTLAssertValidSelector(delegate, finishedSelector, @encode(GTLServiceTicket *), @encode(GTLObject *), @encode(NSError *), 0);
  ...
  GTLAssertValidSelector(delegate, ticket.uploadProgressSelector,
      @encode(GTLServiceTicket *), @encode(unsigned long long),
      @encode(unsigned long long), 0);
  GTLAssertValidSelector(delegate, ticket.retrySelector,
  ...
  GTLFetcherService *fetcherService = self.fetcherService;
  GTLFetcher *fetcher;
...
#if GTL_USE_SESSION_FETCHER
    __block GTLFetcher *fetcherRef = fetcher;
...
- (void)invokeProgressCallbackForTicket:(GTLServiceTicket *)ticket
  ...
  if (progressSelector) {
    ...
    GTLFetcher *fetcher = ticket.objectFetcher;
...
- (void)objectFetcher:(GTLFetcher *)fetcher finishedWithData:(NSData *)data error:(NSError *)error {
...
- (BOOL)objectFetcher:(GTLFetcher *)fetcher willRetry:(BOOL)willRetry forError:(NSError *)error {
...
- (BOOL)invokeRetrySelector:(SEL)retrySelector
  ...
  if ([delegate respondsToSelector:retrySelector]) {
    // Unlike the retry selector invocation in GTLFetcher, this invocation
...
- (void)setUserAgent:(NSString *)userAgent {
  ...
  NSString *str = GTLCleanedUserAgentString(userAgent);
