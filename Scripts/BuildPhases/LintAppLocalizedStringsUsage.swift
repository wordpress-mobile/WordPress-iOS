...
// MARK: PBXObject and subtypes definitions
...
/// Wrapper to decode PBXObject based on their `isa` field
@propertyWrapper
struct PBXObjectWrapper: Decodable, CustomDebugStringConvertible {
    ...
    init(from decoder: Decoder) throws {
        // Try to decode all known types in turn until we find one that succeeds.
...
// MARK: Root type of `.pbxproj` files

struct PBXProjFile: Decodable {
    let rootObject: ObjectUUID
    let objects: [String: PBXObjectWrapper]

    struct ConsistencyError: Swift.Error { let message: String }

    // Convenience methods

    func object<T: PBXObject>(id: ObjectUUID) throws -> T {
        guard let wrapped = objects[id] else { throw ConsistencyError(message: "Unable to find object with UUID \(id)") }
        guard let obj = wrapped.wrappedValue as? T else {
            throw ConsistencyError(message: "Object with UUID \(id) was expected to be of type \(T.self) but found \(wrapped.wrappedValue) instead.")
        }
        return obj
    }
...
// MARK: -
// MARK: Main Object representing and decoding an Xcodeproj project

public class Xcodeproj {
    let projectURL: URL // "<dir>/<Project>.xcodeproj/project.pbxproj"
    let pbxproj: PBXProjFile
    var projectDir: URL { projectURL.deletingLastPathComponent().deletingLastPathComponent() }
    ...
    // Convenience methods redirecting to the wrapped PBXProjFile
    ...
    //: Logic to resolve the full path / URL of a PBXBuildFIle and PBXFileReference
    ...
    private func resolveURL<T: PBXReference>(objectUUID: ObjectUUID, object: T) -> URL {
        if objectUUID == self.pbxproj.rootProject.mainGroup { return URL(fileURLWithPath: ".", relativeTo: projectDir) }
        ...
        case .projectRoot:
            return object.path.map { URL(fileURLWithPath: $0, relativeTo: projectDir) } ?? projectDir
...
let targetsToLint: [PBXNativeTarget]
...
for target in targetsToLint {
    let files: [PBXBuildFile] = project.buildFiles(for: target)
