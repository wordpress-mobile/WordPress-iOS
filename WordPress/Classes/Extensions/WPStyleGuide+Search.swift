...
extension WPStyleGuide {
    ...
    @objc public class func configureSearchBar(_ searchBar: UISearchBar) {
        ...
        searchBar.barTintColor = .neutral(shade: .shade10)
        searchBar.layer.borderColor = UIColor.neutral(shade: .shade10).cgColor
    ...
    @objc public class func configureSearchBarAppearance() {
        ...
        barButtonItemAppearance.tintColor = .neutral(shade: .shade70)

        // We have to manually tint these images, as we want them
        // a different color from the search bar's cursor (which uses `tintColor`)
        let cancelImage = UIImage(named: "icon-clear-searchfield")?.imageWithTintColor(.neutral(shade: .shade30))
        let searchImage = UIImage(named: "icon-post-list-search")?.imageWithTintColor(.neutral(shade: .shade30))
    ...
    @objc public class func configureSearchBarTextAppearance() {
        ...
        UITextField.appearance(whenContainedInInstancesOf: [UISearchBar.self]).defaultTextAttributes =
            (WPStyleGuide.defaultSearchBarTextAttributesSwifted(.neutral(shade: .shade70)))
        ...
        let attributedPlaceholderText = NSAttributedString(string: placeholderText,
                                                           attributes: WPStyleGuide.defaultSearchBarTextAttributesSwifted(.neutral(shade: .shade30)))
