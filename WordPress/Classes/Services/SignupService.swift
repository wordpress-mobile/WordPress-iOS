...
public class SignupService : LocalCoreDataService
{
    ...
    func validateWPComBlogWithParams(params: SigninParams,
        ...
        remote.validateWPComBlogWithUrl(params.url,
                                        ...
                                        success:{ (responseDictionary) in
                                            self.createWPComUserWithParams(params, status: status, success: success, failure: failure)
                                        },
                                        failure: failure)
    }
    ...
    func createWPComUserWithParams(params: SigninParams,
        ...
        remote.createWPComAccountWithEmail(params.email,
                                            ...
                                            success: { (responseDictionary) in
                                                ...
                                                self.signinWPComUserWithParams(params, status: status, success: success, failure: failure)
                                            },
                                            failure: failure)
    }
        ...
        client.authenticateWithUsername(params.username,
                                        ...
                                        success: { (authToken: String?) in
                                            ...
                                            guard let authToken = authToken else {
                                                ...
                                                let signupError = SignupError.InvalidResponse
                                                let error = NSError(domain: signupError.domain, code: signupError.code, userInfo: nil)
                                                failure(error: error)
                                                return
                                            }
                                            ...
                                            self.createWPComBlogForParams(params, account: account, status: status, success: success, failure: failure)
                                        },
                                        failure: failure)
    }
    ...
    func createWPComBlogForParams(params: SigninParams,
        ...
        guard let api = account.restApi else {
            ...
            let signupError = SignupError.MissingRESTAPI
            let error = NSError(domain: signupError.domain, code: signupError.code, userInfo: nil)
            failure(error: error)
            return
        }
    ...
    func createBlogFromBlogOptions(blogOptions: [String: AnyObject], failure: SignupFailureBlock) -> Blog? {
        ...
        guard let defaultAccount = accountService.defaultWordPressComAccount() else {
            ...
            let signupError = SignupError.MissingDefaultWPComAccount
            let error = NSError(domain: signupError.domain, code: signupError.code, userInfo: nil)
            failure(error: error)
            return nil
        }
