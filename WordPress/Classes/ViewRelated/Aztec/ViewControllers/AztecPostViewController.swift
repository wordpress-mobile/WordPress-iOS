        ...
        if let videoSrcURL = videoAttachment.url,
           ...
           let videoPressID = videoSrcURL.host {
            ...
            mediaService.getMetadataFromVideoPressID(videoPressID, in: self.post.blog, success: { (metadata) in
                videoAttachment.updateURL(metadata.getURLWithToken(url: metadata.originalURL) ?? metadata.originalURL)
                if let posterURL = metadata.getURLWithToken(url: metadata.posterURL) ?? metadata.posterURL {
