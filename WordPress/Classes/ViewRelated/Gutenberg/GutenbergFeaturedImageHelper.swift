...
class GutenbergFeaturedImageHelper: NSObject {
    ...
    func setFeaturedImage(mediaID: Int32) {
        ...
        if mediaID == GutenbergFeaturedImageHelper.mediaIdNoFeaturedImageSet {
            gutenberg.showNotice(AppLocalizedString("Removed as featured image", comment: "Notice confirming that an image has been removed as the post's featured image."))
            WPAnalytics.track(event, properties: [
                "via": "gutenberg",
                "action": "removed"
            ])
        } else {
            gutenberg.showNotice(AppLocalizedString("Set as featured image", comment: "Notice confirming that an image has been set as the post's featured image."))
            WPAnalytics.track(event, properties: [
                "via": "gutenberg",
                "action": "added"
            ])
