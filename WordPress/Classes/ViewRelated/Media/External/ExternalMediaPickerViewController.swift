...
final class ExternalMediaPickerViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UISearchResultsUpdating {
    ...
    private var items: [ExternalMedia] = []
    private let dataSource: ExternalImagePickerDataSource

    init(dataSource: ExternalImagePickerDataSource) {
    ...
    override func viewDidLoad() {
        ...
        // TODO: figure out what image loader to use (MemoryCache; not disk cache, decompression)
    ...
    private func configureCollectionView() {
        ...
        Task {
            do {
                self.items = try await dataSource.loadMedia(searchTerm: "")
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        // TODO: imppement searching
    }
}

// TODO: rename/load move to external file
protocol ExternalMedia {
    var url: URL { get }
}

protocol ExternalImagePickerDataSource {
    func loadMedia(searchTerm: String) async throws -> [ExternalMedia]
}

// TODO: add paging?
// TODO: check what else is missing from the web version
final class TenorDataSource2: ExternalImagePickerDataSource {
    func loadMedia(searchTerm: String) async throws -> [ExternalMedia] {
        demoPhotosURLs.map(TenorMedia2.init)
    }
}

struct TenorMedia2: ExternalMedia {
    let url: URL
}
