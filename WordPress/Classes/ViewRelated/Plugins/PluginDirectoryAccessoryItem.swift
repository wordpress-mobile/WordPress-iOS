...
struct PluginDirectoryAccessoryItem {
    ...
    private static func automanaged() -> UIView {
        let color = AppColor.neutral(.shade40)
    ...
    private static func stars(count: Double) -> UIView {
        ...
        for i in 1...Constants.totalStarsCount {
            ...
            if i <= Int(wholeStars) {
                ...
                color = AppColor.primary(.shade40)
            } else {
                image = .gridicon(.starOutline, size: Constants.starImageSize)
                color = AppColor.neutral(.shade10)
            }
    ...
    private static func halfStar(size: CGSize) -> UIView {
        let color = AppColor.primary(.shade40)
        ...
        rightHalf.tintColor = AppColor.neutral(.shade10)
