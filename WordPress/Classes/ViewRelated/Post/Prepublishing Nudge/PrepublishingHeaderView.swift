...
class PrepublishingHeaderView: UITableViewHeaderFooterView, NibLoadable {
    ...
    private enum Constants {
        ...
        static let title = AppLocalizedString("Publishing To", comment: "Label that describes in which blog the user is publishing to")
        static let close = AppLocalizedString("Close", comment: "Voiceover accessibility label informing the user that this button dismiss the current view")
        static let doubleTapToDismiss = AppLocalizedString("Double tap to dismiss", comment: "Voiceover accessibility hint informing the user they can double tap a modal alert to dismiss it")
    }
}
