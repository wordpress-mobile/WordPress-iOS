...
@objc open class ReaderCardDiscoverAttributionView: UIView, NibLoadable {
    ...
    fileprivate func stringForPostAttribution(_ authorName: String?, blogName: String?) -> String {
        ...
        if (authorName != nil) && (blogName != nil) {
            let pattern = AppLocalizedString("Originally posted by %@ on %@",
            ...
            let pattern = AppLocalizedString("Originally posted by %@",
            ...
            let pattern = AppLocalizedString("Originally posted on %@",
    ...
    fileprivate func patternForSiteAttribution(_ verbose: Bool) -> String {
        var pattern: String
        if verbose {
            pattern = AppLocalizedString("Visit %@ for more", comment: "A call to action to visit the specified blog.  The '%@' characters are a placholder for the blog name.")
        } else {
            pattern = AppLocalizedString("Visit %@", comment: "A call to action to visit the specified blog.  The '%@' characters are a placholder for the blog name.")
        }
        return pattern
    }
