...
class ReaderRelativeTimeFormatter: NSObject {
    ...
    public func string(from date: Date) -> String {
        ...
        let deltaComponents: DateComponents = calendar.dateComponents([.minute, .hour, .day], from: date, to: now)
        ...
        let dateYear: Int = calendar.component(.year, from: date)
        let nowYear: Int = calendar.component(.year, from: now)
        ...
        if let day: Int = deltaComponents.day, day >= 7 {
            return recentDateFormatter.string(from: date)
        }

        // If the date is within a week display a relative time in days:
        // 1d, 3d, 6d
        if let day: Int = deltaComponents.day, day > 0 {
            let format: String = NSLocalizedString("%dd", comment: "Relative time format with day unit abbreviation (d): Xd, 1d, 3d, 6d")
            return String(format: format, day)
        }

        // If the date is within 24 hours display the relative time in hours
        // 1h, 12h, 23h
        if let hour: Int = deltaComponents.hour, hour > 0 {
            let format: String = NSLocalizedString("%dh", comment: "Relative time format with hour unit abbreviation (h): Xh, 1h, 10h, 23h")
            return String(format: format, hour)
        }

        // If the date is within an hour, display the relative time in minutes
        // 1m, 50m, 59m
        if let minute: Int = deltaComponents.minute, minute >= 0 {
            // If the minute value is 0 round it up to 1 so we don't display "0m"
            // Example being if the date is 30 seconds old
            let value: Int = (minute == 0) ? 1 : minute

            let format: String = NSLocalizedString("%dm", comment: "Relative time format with minute unit abbreviation (m): Xm, 1m, 10m, 59m")
