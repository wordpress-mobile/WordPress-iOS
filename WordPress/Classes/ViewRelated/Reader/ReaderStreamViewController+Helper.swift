...
extension ReaderStreamViewController {
    ...
    static let defaultResponse = NoResultsResponse(
        title: AppLocalizedString("No recent posts", comment: "A message title"),
        message: AppLocalizedString("No posts have been made recently", comment: "A default message shown whe the reader can find no post to display"))
    ...
    public class func responseForNoResults(_ topic: ReaderAbstractTopic) -> NoResultsResponse {
        ...
        if ReaderHelpers.topicIsFollowing(topic) {
            return NoResultsResponse(
                title: AppLocalizedString("Welcome to the Reader", comment: "A message title"),
                message: AppLocalizedString("Recent posts from blogs and sites you follow will appear here.", comment: "A message explaining the Following topic in the reader")
            )
        }
        ...
        if ReaderHelpers.topicIsLiked(topic) {
            return NoResultsResponse(
                title: AppLocalizedString("Nothing liked yet", comment: "A message title"),
                message: AppLocalizedString("Posts that you like will appear here.", comment: "A message explaining the Posts I Like feature in the reader")
            )
        }
        ...
        if ReaderHelpers.isTopicTag(topic) {
            return NoResultsResponse(
                title: AppLocalizedString("No recent posts", comment: "A message title"),
                message: AppLocalizedString("No posts have been made recently with this tag.", comment: "Message shown whent the reader finds no posts for the chosen tag")
            )
        }
        ...
        if ReaderHelpers.isTopicSite(topic) {
            return NoResultsResponse(
                title: AppLocalizedString("No posts", comment: "A message title"),
                message: AppLocalizedString("This site has not posted anything yet. Try back later.", comment: "Message shown when the reader finds no posts for the chosen site")
            )
        }
        ...
        if ReaderHelpers.isTopicList(topic) {
            return NoResultsResponse(
                title: AppLocalizedString("No recent posts", comment: "A message title"),
                message: AppLocalizedString("The sites in this list have not posted anything recently.", comment: "Message shown when the reader finds no posts for the chosen list")
            )
        }

        // if search topic
        if ReaderHelpers.isTopicSearchTopic(topic) {
            let message = AppLocalizedString("No posts found matching %@ in your language.", comment: "Message shown when the reader finds no posts for the specified search phrase. The %@ is a placeholder for the search phrase.")
            return NoResultsResponse(
                title: AppLocalizedString("No posts found", comment: "A message title"),
...
extension ReaderStreamViewController {
    ...
    func configureNoResultsViewForSavedPosts() {
        ...
        let noResultsResponse = NoResultsResponse(title: AppLocalizedString("No Saved Posts",
                                                                           comment: "Message displayed in Reader Saved Posts view if a user hasn't yet saved any posts."),
                                                  message: AppLocalizedString("Tap [bookmark-outline] to save a post to your list.",
