...
class SiteStatsInsightsViewModel: Observable {
    ...
    private func startFetchingPeriodOverview() {
        periodStore.actionDispatcher.dispatch(
            PeriodAction.refreshPeriodOverviewData(
    ...
    func tableViewSnapshot() -> ImmuTableDiffableDataSourceSnapshot {
        ...
        if insightsToShow.isEmpty || (fetchingFailed() && !containsCachedData()) {
            return snapshot
        }

        for insightType in insightsToShow {
            let errorBlock = { statSection in
                StatsErrorRow(rowStatus: .error, statType: .insights, statSection: statSection)
            }

            switch insightType {
            case .viewsVisitors:
                let viewsVisitorsSection = ImmuTableDiffableSectionID.insight(.viewsVisitors)
                let viewsVisitorsRows = blocks(
                    for: .viewsVisitors,
                    type: .period,
                    ...
                    block: { [weak self] in
                        self?.overviewTableRows() ?? [errorBlock(.insightsViewsVisitors)]
                    },
                    loading: {
                        [StatsGhostChartImmutableRow(statSection: .insightsViewsVisitors)]
                    },
                    error: {
                        [errorBlock(.insightsViewsVisitors)]
                    }
                )
            ...
            case .growAudience:
                ...
                let growAudienceSection = ImmuTableDiffableSectionID.insight(.growAudience)
                let growAudienceRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .growAudience,
                    type: .insights,
                    ...
                    loading: {
                        [StatsGhostGrowAudienceImmutableRow(statSection: InsightType.growAudience.statSection)]
                    },
                    error: {
                        [errorBlock(nil)]
                    }
                ...
                snapshot.appendItems(
                    growAudienceRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: growAudienceSection
                )
            case .latestPostSummary:
                let latestPostSummarySection = ImmuTableDiffableSectionID.insight(.latestPostSummary)
                let latestPostSummaryRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .latestPostSummary,
                    type: .insights,
                    status: insightsStore.lastPostSummaryStatus,
                    block: {
                        [
                            LatestPostSummaryRow(
                                summaryData: insightsStore.getLastPostInsight(),
                    ...
                    loading: {
                        [StatsGhostChartImmutableRow(statSection: .insightsLatestPostSummary)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    latestPostSummaryRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: latestPostSummarySection
                )
            case .allTimeStats:
                let allTimeStatsSection = ImmuTableDiffableSectionID.insight(.allTimeStats)
                let allTimeStatsRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .allTimeStats,
                    type: .insights,
                    status: insightsStore.allTimeStatus,
                    block: {
                        [
                            TwoColumnStatsRow(
                                dataRows: createAllTimeStatsRows(),
                    ...
                    loading: {
                        [StatsGhostTwoColumnImmutableRow(statSection: .insightsAllTime)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    allTimeStatsRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: allTimeStatsSection
                )
            case .likesTotals:
                let likesTotalsSection = ImmuTableDiffableSectionID.insight(.likesTotals)
                let likesTotalsRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .likesTotals,
                    type: .period,
                    status: periodStore.timeIntervalsSummaryStatus,
                    checkingCache: { [weak self] in
                    ...
                    block: {
                        [
                            TotalInsightStatsRow(
                                ...
                                statSection: .insightsLikesTotals,
                                siteStatsInsightsDelegate: siteStatsInsightsDelegate
                            )
                        ]
                    },
                    loading: {
                        [StatsGhostTwoColumnImmutableRow(statSection: .insightsLikesTotals)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    likesTotalsRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: likesTotalsSection
                )
            case .commentsTotals:
                let commentsTotalsSection = ImmuTableDiffableSectionID.insight(.commentsTotals)
                let commentsTotalsRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .commentsTotals,
                    type: .period,
                    status: periodStore.timeIntervalsSummaryStatus,
                    checkingCache: { [weak self] in
                    ...
                    block: {
                        [
                            TotalInsightStatsRow(
                                dataRow: createCommentsTotalInsightsRow(),
                                statSection: .insightsCommentsTotals,
                                siteStatsInsightsDelegate: siteStatsInsightsDelegate
                            )
                        ]
                    },
                    loading: {
                        [StatsGhostTwoColumnImmutableRow(statSection: .insightsCommentsTotals)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    commentsTotalsRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: commentsTotalsSection
                )
            case .followersTotals:
                let followersTotalsSection = ImmuTableDiffableSectionID.insight(.followersTotals)
                let followersTotalsRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .followersTotals,
                    type: .insights,
                    ...
                    block: {
                        [
                            TotalInsightStatsRow(
                                dataRow: createFollowerTotalInsightsRow(),
                    ...
                    loading: {
                        [StatsGhostTwoColumnImmutableRow(statSection: .insightsFollowerTotals)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    followersTotalsRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: followersTotalsSection
                )
            case .mostPopularTime:
                let mostPopularTimeSection = ImmuTableDiffableSectionID.insight(.mostPopularTime)
                let mostPopularTimeRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .mostPopularTime,
                    type: .insights,
                    status: insightsStore.annualAndMostPopularTimeStatus,
                    block: {
                        [
                            MostPopularTimeInsightStatsRow(
                                data: createMostPopularStatsRowData(),
                                siteStatsInsightsDelegate: nil,
                    ...
                    loading: {
                        [StatsGhostTwoColumnImmutableRow(statSection: .insightsMostPopularTime)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    mostPopularTimeRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: mostPopularTimeSection
                )
            case .tagsAndCategories:
                let tagsAndCategoriesSection = ImmuTableDiffableSectionID.insight(.tagsAndCategories)
                let tagsAndCategoriesRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .tagsAndCategories,
                    type: .insights,
                    status: insightsStore.tagsAndCategoriesStatus,
                    block: {
                        [
                            TopTotalsInsightStatsRow(
                                itemSubtitle: StatSection.insightsTagsAndCategories.itemSubtitle,
                    ...
                    loading: {
                        [StatsGhostTopImmutableRow(statSection: .insightsTagsAndCategories)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    tagsAndCategoriesRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: tagsAndCategoriesSection
                )
            case .annualSiteStats:
                let annualSiteStatsSection = ImmuTableDiffableSectionID.insight(.annualSiteStats)
                let annualSiteStatsRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .annualSiteStats,
                    type: .insights,
                    status: insightsStore.annualAndMostPopularTimeStatus,
                    block: {
                    ...
                    loading: {
                        [StatsGhostTwoColumnImmutableRow(statSection: .insightsAnnualSiteStats)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    annualSiteStatsRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: annualSiteStatsSection
                )
            case .comments:
                let commentsSection = ImmuTableDiffableSectionID.insight(.comments)
                let commentsRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .comments,
                    type: .insights,
                    ...
                    loading: {
                        [StatsGhostTabbedImmutableRow(statSection: .insightsCommentsAuthors)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    commentsRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: commentsSection
                )
            case .followers:
                let followersSection = ImmuTableDiffableSectionID.insight(.followers)
                let followersRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .followers,
                    type: .insights,
                    status: insightsStore.followersTotalsStatus,
                    block: {
                    ...
                    loading: {
                        [StatsGhostTabbedImmutableRow(statSection: .insightsFollowersWordPress)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    followersRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: followersSection
                )
            case .todaysStats:
                let todaysStatsSection = ImmuTableDiffableSectionID.insight(.todaysStats)
                let todaysStatsRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .todaysStats,
                    type: .insights,
                    ...
                    block: {
                        [
                            TwoColumnStatsRow(
                                dataRows: createTodaysStatsRows(),
                    ...
                    loading: {
                        [StatsGhostTwoColumnImmutableRow(statSection: .insightsTodaysStats)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    todaysStatsRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: todaysStatsSection
                )
            case .postingActivity:
                let postingActivitySection = ImmuTableDiffableSectionID.insight(.postingActivity)
                let postingActivityRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .postingActivity,
                    type: .insights,
                    status: insightsStore.postingActivityStatus,
                    block: {
                        [createPostingActivityRow()]
                    },
                    loading: {
                        [StatsGhostPostingActivitiesImmutableRow(statSection: .insightsPostingActivity)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    postingActivityRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: postingActivitySection
                )
            case .publicize:
                let publicizeSection = ImmuTableDiffableSectionID.insight(.publicize)
                let publicizeRows: [any StatsHashableImmuTableRow] = blocks(
                    for: .publicize,
                    type: .insights,
                    ...
                    block: {
                        [
                            TopTotalsInsightStatsRow(
                                itemSubtitle: StatSection.insightsPublicize.itemSubtitle,
                    ...
                    loading: {
                        [StatsGhostTopImmutableRow(statSection: .insightsPublicize)]
                    },
                    error: {
                ...
                snapshot.appendItems(
                    publicizeRows.map { AnyHashableImmuTableRow(immuTableRow: $0) },
                    toSection: publicizeSection
                )
            default:
                break
            }
        ...
        let addInsightSection = ImmuTableDiffableSectionID.addInsight
        let addInsightRow = AddInsightRow(
    ...
    func fetchingFailed() -> Bool {
        insightsStore.fetchingFailed(for: .insights)
    }

    func containsCachedData() -> Bool {
        insightsStore.containsCachedData(for: insightsToShow)
    }

    func annualInsightsYear() -> String? {
        insightsStore.getAnnualAndMostPopularTime()?.years?.last?.year
    }
    ...
    static func intervalData(
        _ statsSummaryTimeIntervalData: StatsSummaryTimeIntervalData?,
        summaryType: StatsSummaryType,
        periodEndDate: Date? = nil
    ) -> (count: Int, prevCount: Int, difference: Int, percentage: Int) {
        guard let statsSummaryTimeIntervalData else {
            return (0, 0, 0, 0)
        }

        let splitSummaryTimeIntervalData = SiteStatsInsightsViewModel.splitStatsSummaryTimeIntervalData(
...
private extension SiteStatsInsightsViewModel {
    ...
    struct AllTimeStats {
        ...
        static let bestViewsEverTitle = NSLocalizedString(
            "Best views ever",
            comment: "All Time Stats 'Best views ever' label"
        )
    }
    ...
    func overviewTableRows() -> [any StatsHashableImmuTableRow] {
        ...
        return SiteStatsImmuTableRows.viewVisitorsImmuTableRows(
            mostRecentChartData,
    ...
    func createAllTimeStatsRows() -> [StatsTwoColumnRowData] {
        ...
        let totalCounts =
            allTimeInsight.viewsCount + allTimeInsight.visitorsCount + allTimeInsight.postsCount
        ...
        dataRows.append(
            StatsTwoColumnRowData(
                leftColumnName: AllTimeStats.viewsTitle,
        ...
        dataRows.append(
            StatsTwoColumnRowData(
                leftColumnName: AllTimeStats.postsTitle,
    ...
    func createMostPopularStatsRowData() -> StatsMostPopularTimeData? {
        ...
        let dayPercentage = String(
            format: MostPopularStats.viewPercentage,
            mostPopularStats.mostPopularDayOfWeekPercentage.percentageString()
        )
        let hourPercentage = String(
            format: MostPopularStats.viewPercentage,
            mostPopularStats.mostPopularHourPercentage.percentageString()
        )

        return StatsMostPopularTimeData(
            mostPopularDayTitle: MostPopularStats.bestDay,
            mostPopularTimeTitle: MostPopularStats.bestHour,
            mostPopularDay: dayString,
            mostPopularTime: timeString.uppercased(),
            dayPercentage: dayPercentage,
            timePercentage: hourPercentage
        )
    }

    func createLikesTotalInsightsRow() -> StatsTotalInsightsData {
        StatsTotalInsightsData.createTotalInsightsData(
            periodSummary: periodStore.getSummary(),
    ...
    func createFollowerTotalInsightsRow() -> StatsTotalInsightsData {
        ...
        if data.count < Constants.followersGuideLimit {
            let guideText = NSLocalizedString(
                "Commenting on other blogs is a great way to build attention and followers for your new site.",
    ...
    func createTodaysStatsRows() -> [StatsTwoColumnRowData] {
        ...
        let totalCounts =
            todaysStats.viewsCount + todaysStats.visitorsCount + todaysStats.likesCount + todaysStats.commentsCount
        ...
        dataRows.append(
            StatsTwoColumnRowData(
                leftColumnName: TodaysStats.viewsTitle,
        ...
        dataRows.append(
            StatsTwoColumnRowData(
                leftColumnName: TodaysStats.likesTitle,
    ...
    func createAnnualRows() -> [StatsTwoColumnRowData] {
        ...
        dataRows.append(
            StatsTwoColumnRowData(
                leftColumnName: AnnualSiteStats.year,
        ...
        dataRows.append(
            StatsTwoColumnRowData(
                leftColumnName: AnnualSiteStats.totalComments,
        ...
        dataRows.append(
            StatsTwoColumnRowData(
                leftColumnName: AnnualSiteStats.totalLikes,
        ...
        dataRows.append(
            StatsTwoColumnRowData(
                leftColumnName: AnnualSiteStats.totalWords,
    ...
    func createCommentsRow() -> TabbedTotalsStatsRow {
        TabbedTotalsStatsRow(
            tabsData: [
                tabDataForCommentType(.insightsCommentsAuthors),
...
extension SiteStatsInsightsViewModel: AsyncBlocksLoadable {
    ...
    ///
    public static func splitStatsSummaryTimeIntervalData(
        _ statsSummaryTimeIntervalData: StatsSummaryTimeIntervalData,
        periodEndDate: Date? = nil
        ...
        if let periodEndDate {
            summaryData = splitStatsSummaryTimeIntervalDataPadForward(
                statsSummaryTimeIntervalData,
                periodEndDate: periodEndDate
            )
        }
    ...
    ///
    public static func splitStatsSummaryTimeIntervalDataPadForward(
        _ statsSummaryTimeIntervalData: StatsSummaryTimeIntervalData,
        periodEndDate: Date
    ) -> [StatsSummaryData] {
        var summaryData = statsSummaryTimeIntervalData.summaryData
        let daysElapsedSincePeriodEndDate =
            Calendar.autoupdatingCurrent
            .dateComponents([.day], from: statsSummaryTimeIntervalData.periodEndDate, to: periodEndDate).day ?? 0
        if daysElapsedSincePeriodEndDate > 0 {
            for i in 1...daysElapsedSincePeriodEndDate {
                if let date = Calendar.autoupdatingCurrent.date(
                    byAdding: .day,
                    value: i,
                    to: statsSummaryTimeIntervalData.periodEndDate
                ) {
                    summaryData.append(
                        StatsSummaryData(
                            period: .day,
    ...
    public static func splitStatsSummaryData(_ summaryData: [StatsSummaryData]) -> [StatsSummaryTimeIntervalDataAsAWeek]
    {
        ...
        case let count where count == Constants.fourteenDays:
            ...
            return createStatsSummaryTimeIntervalDataAsAWeeks(
                summaryData: Array(summaryData[0..<Constants.fourteenDays])
            )
        case let count where count > Constants.fourteenDays:
            // when more than 14 rows we take the last 14 rows for most recent data
            return createStatsSummaryTimeIntervalDataAsAWeeks(
                summaryData: Array(summaryData[count - Constants.fourteenDays..<count])
            )
        case let count where count < Constants.fourteenDays:
    ...
    public static func createStatsSummaryTimeIntervalDataAsAWeeks(
        ...
        return [
            StatsSummaryTimeIntervalDataAsAWeek.thisWeek(data: thisWeekTimeIntervalData),
