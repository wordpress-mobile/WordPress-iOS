...
- App Store Connect allows only **one build per train** (marketing version) in beta review at a time. Right after a version bump, a train's first build can sit in review for hours, so a later build in the same train can't be submitted until it clears — App Store Connect returns *"Another build in the same train is already in beta review."* This is treated as a **non-fatal skip**, not a failure (`beta_review_in_progress_error?`): the job stays green and Slack reports `:fast_forward: skipped` for that app. A build only ever enters a beta group after it clears review, so a public-beta candidate has already been reviewed — the picked app itself never hits this skip. Only its cross-app partner can: WordPress and Jetpack are separate App Store Connect records Apple approves minutes-to-days apart, so it's normal for one to submit while the other's first-of-train build is still in review and skips. Reconciliation differs by lane: the **nightly** job re-promotes the newest build automatically on a later run once the train clears; the **weekly public** promotion is human-picked, so a skipped app is *not* auto-reconciled and must be re-triggered by hand — the Slack header names any skipped app (`:rocket: Promoted … — skipped Jetpack (already in review)`) so a green run isn't mistaken for a complete one. The lanes deliberately **don't** expire the in-flight submission to jump the queue (`reject_build_waiting_for_review: false`) — resubmitting a newer build every night would reset Apple's review clock and could keep a new train's first build from ever being approved.
