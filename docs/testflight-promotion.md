...
- App Store Connect allows only **one build per train** (marketing version) in beta review at a time. Right after a version bump, a train's first build can sit in review for hours, so a later build in the same train can't be submitted until it clears — App Store Connect returns *"Another build in the same train is already in beta review."* This is treated as a **non-fatal skip**, not a failure (`beta_review_in_progress_error?`): a build is already on its way to those testers, and the newest build is picked up on a later run once the train clears. The job stays green and Slack reports `:fast_forward: skipped` for that app. The lanes deliberately **don't** expire the in-flight submission to jump the queue (`reject_build_waiting_for_review: false`) — resubmitting a newer build every night would reset Apple's review clock and could keep a new train's first build from ever being approved. WordPress and Jetpack are separate App Store Connect records Apple approves minutes-to-days apart, so it's normal for one to submit while the other skips; they reconcile on a later run.
